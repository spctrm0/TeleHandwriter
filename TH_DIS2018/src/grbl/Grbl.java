...
public class Grbl {
	...
	public void backOff(int _periodMsec) {
		if (isNeedToBeBackOff) {
			if (getTimePassedMsec() >= _periodMsec) {
				reserveBackOffCmd();
				isNeedToBeBackOff = false;
			}
		}
	}
	...
	public void writeReservedCmdAsPossible() {
		while (bfrSize <= bfrSizeMx && reservedCmd.size() > 0) {
			String cmd_;
			cmd_ = reservedCmd.get(0).toString();
