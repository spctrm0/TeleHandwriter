public class GrblComm {
  ...
  private void writeReservedCmdAsPossible() {
    while (bfrSize <= bfrSizeMx && reservedPreDefinedCmd.size() > 0) {
      ...
      if (bfrSize + cmd_.length() <= bfrSizeMx) {
        ...
        bfrSize += cmd_.length();
        grblBfr.add(cmd_);
        if(cmd_.contains(";") == true) {
          int first = cmd_.indexOf(";");
          int second = cmd_.indexOf(">", first);
          String tiltCmd_ = cmd_.substring(first+1, second+1);
          if(tiltSerial != null) {
            tiltSerial.write(tiltCmd_);  
            System.out.print("<TLT> Send to tiltSerial" + tiltCmd_ + "\n");
          }
        }
    ...
    if (gate) {
      while (bfrSize <= bfrSizeMx && reservedCmd.size() > 0 && reservedPreDefinedCmd.size() == 0) {
        ...
        if (bfrSize + cmd_.length() <= bfrSizeMx) {
          ...
          bfrSize += cmd_.length();
          grblBfr.add(cmd_);
          if(cmd_.contains(";") == true && cmd_.contains("M3S") == false) {
            int first = cmd_.indexOf(";");
            int second = cmd_.indexOf(">", first);
            String tiltCmd_ = cmd_.substring(first+1, second+1);
            if(tiltSerial != null) {
  ...
  public void read(String _msg) {
    if (_msg.equals("ok") || _msg.contains("error:")) {
      ...
      bfrSize -= cmd_.length();
      grblBfr.remove(0);
      String print_ = "<GRBL>\t\"" + _msg + "\" with " + receivedMsg.size() + " message(s)... " + cmd_ + " reservedCmd size : " + reservedCmd.size() + " , bfrSize : " + bfrSize + "\n";
      for (int i = 0; i < receivedMsg.size(); i++)
        print_ += "\0 ;\t:(" + i + "): " + receivedMsg.get(i) + "\n";
