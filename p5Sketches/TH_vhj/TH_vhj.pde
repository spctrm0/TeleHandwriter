...
public void exit() {
  ...
  tiltCmd_ += "90";
  tiltCmd_ += ">";
  grblComm.terminateCmd(cmd_, tiltCmd_);
  super.exit();
}
...
void connectTiltSerial() {
  if(serialComm.getPortId() != -1 && tiltSerial == null) {
    int _connectedPortId = serialComm.getPortId();
    int _unusedPort = -1;
    
    for(int i = 0; i < Serial.list().length; i++) {
      if(!Serial.list()[i].equals("COM1") && i != _connectedPortId) {
        _unusedPort = i;  
      }
    }
    
    tiltSerial = new Serial(this, Serial.list()[_unusedPort], 9600);
    println("<TLT> Tilt Serial is Connected: " + tiltSerial.port.getPortName());
  }
}
