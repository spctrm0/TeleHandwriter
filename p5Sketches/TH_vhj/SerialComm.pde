public class SerialComm {
  ...
  public void writeTilt(String _tilt) {
    if (tiltSerial != null) {
      tiltSerial.write(_tilt);
    }
  }
  ...
  //added for returning port info
  public int getPortId() {
  
    int _foundPortId = -1;
    
    if(isConnected) {
      String _portName = serial.port.getPortName();
      
      for(int i=0; i<Serial.list().length; i++) {
        if(_portName.equals(Serial.list()[i])) {
          _foundPortId = i;  
        }
      }
    }
